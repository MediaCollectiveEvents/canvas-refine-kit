...
const WhoAttendsSection: React.FC<WhoAttendsSectionProps> = ({ section }) => {
  ...
  // Intro paragraphs based on CMS values (with fallbacks)
  const intro: string[] = [
  ...
  // Default cards if CMS highlights are missing
    ...
    <section className="py-24 md:py-32 bg-background border-t border-white/5">
      <div className="container mx-auto px-6 max-w-4xl text-center">
        ...
        <div className="grid gap-8 md:grid-cols-3">
          {effectiveHighlights.map((card, idx) => {
            const Icon = iconMap[card.icon ?? "globe"] ?? iconMap["globe"];
