  ...
  once: true
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
      ...
      <div className={`relative overflow-hidden rounded-2xl ${index % 2 === 1 ? 'lg:order-2' : ''}`}>
        <img src={event.image} alt={event.title} className="w-full h-auto object-contain transition-transform duration-700 group-hover:scale-105" />
      ...
      <div className={`space-y-6 ${index % 2 === 1 ? 'lg:order-1 lg:text-right' : ''}`}>
        ...
        <button onClick={() => onRegisterClick(event.id === 1 ? "networking-breakfast" : event.id === 2 ? "nab-review" : "mpts-drinks")} className="inline-flex items-center gap-2 bg-primary text-primary-foreground px-6 py-3 rounded-full font-body uppercase tracking-wider text-sm hover:bg-primary/90 transition-all duration-300 group-hover:gap-3">
          Register Interest
      ...
      <section className="py-20 px-6">
        <div className="container mx-auto max-w-6xl">
          ...
          <div className="space-y-24">
            {upcomingEvents.map((event, index) => <EventCard key={event.id} event={event} index={index} onRegisterClick={handleRegisterClick} />)}
          </div>
        </div>
      </section>

      {/* Past Events - only show if there are any */}
      {pastEvents.length > 0 && <section className="py-20 px-6 bg-muted/30">
          <div className="container mx-auto max-w-6xl">
            <motion.div initial={{
          opacity: 0,
          y: 20
          ...
          once: true
            ...
            <div className="space-y-24">
              {pastEvents.map((event, index) => <EventCard key={event.id} event={event} index={index} onRegisterClick={handleRegisterClick} />)}
            </div>
          </div>
        </section>}
      ...
      <section className="py-24 px-6">
        <div className="container mx-auto max-w-4xl text-center">
          ...
          once: true
            ...
            <button onClick={() => setIsFormOpen(true)} className="inline-block bg-primary text-primary-foreground px-8 py-4 rounded-full font-body uppercase tracking-wider text-sm hover:bg-primary/90 transition-colors">
              Register Interest
            </button>
          </motion.div>
        </div>
